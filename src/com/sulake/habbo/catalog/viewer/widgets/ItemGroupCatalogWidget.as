package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class ItemGroupCatalogWidget extends ItemGridCatalogWidget implements ICatalogWidget, IItemGrid
   {
      ...
      {
         var _loc2_:Offer = this.var_627[this.var_1179][this.var_1854];
      ...
      override public function select(param1:IGridItem) : void
      {
         ...
         super.select(param1);
         this.var_1179 = var_998.getGridItemIndex(param1.view);
         var _loc2_:String = this._groupNames[this.var_1179];
         var _loc3_:Array = this.var_627[this.var_1179] as Array;
      ...
      override protected function populateItemGrid() : void
      {
         var _loc1_:* = null;
         var _loc2_:* = null;
         for each(_loc1_ in this.var_627)
         {
            if(this.var_1854 < _loc1_.length)
            {
               _loc2_ = _loc1_[this.var_1854] as Offer;
      ...
      private function createOfferGroups() : void
      {
         ...
         {
            if(_loc1_.pricingModel == Offer.const_555 || _loc1_.pricingModel == Offer.const_554)
            {
               _loc3_ = _loc1_.productContainer.firstProduct;
               if(_loc3_ != null)
               {
                  _loc4_ = _loc3_.productClassId;
                  if(_loc3_.productType == ProductTypeEnum.const_81 || _loc3_.productType == ProductTypeEnum.const_91)
                  {
                     if(_loc3_.furnitureData != null)
                     {
                        ...
                        if(this._groupNames.indexOf(_loc5_) == -1)
                        {
                           ...
                           this.var_627.push(new Array());
                        }
                        _loc6_ = this._groupNames.indexOf(_loc5_);
                        (this.var_627[_loc6_] as Array).push(_loc1_);
                     }
                  }
               }
            }
         }
         for each(_loc2_ in this.var_627)
         {
