package com.sulake.habbo.catalog.marketplace
{
   ...
   public class MarketPlaceLogic implements IMarketPlace
   {
      ...
      public const const_1869:int = 1;
      
      public const const_1770:int = 2;
      
      public const const_2011:int = 3;
      ...
      private var var_2265:MarketplaceItemStats;
      ...
      public function dispose() : void
      {
         ...
         if(this.var_171)
         {
            for each(_loc1_ in this.var_171)
            {
               if(_loc1_ != null)
               {
                  _loc1_.dispose();
      ...
      {
         this.requestOffers(this.var_2264,this.var_2261,this.var_2010,this.var_2266);
      }
      ...
      {
         if(!this.var_171 || !this._catalog || !this._catalog.getPurse())
         {
            return;
         }
         var _loc2_:MarketPlaceOfferData = this.var_171.getValue(param1) as MarketPlaceOfferData;
         if(!_loc2_)
         {
            return;
         }
         ...
         this.showConfirmation(this.const_1869,_loc2_);
      }
      ...
      public function redeemSoldOffers() : void
      {
         ...
         for each(_loc2_ in _loc1_)
         {
            ...
            if(_loc3_.status == MarketPlaceOfferState.const_569)
            {
      ...
      public function onOffers(param1:IMessageEvent) : void
      {
         ...
         if(this.var_171)
         {
            for each(_loc5_ in this.var_171)
            {
               if(_loc5_ != null)
               {
                  _loc5_.dispose();
         ...
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            this.var_171.add(_loc4_.offerId,_loc6_);
         }
      ...
      public function onOwnOffers(param1:IMessageEvent) : void
      {
         ...
         this.var_2262 = _loc3_.creditsWaiting;
      ...
      public function onBuyResult(param1:IMessageEvent) : void
      {
         ...
         {
            item = this.var_171.remove(parser.requestedOfferId);
            if(item != null)
            {
               item.dispose();
            }
         ...
         {
            updateItem = this.var_171.getValue(parser.requestedOfferId) as MarketPlaceOfferData;
            if(updateItem)
            {
               updateItem.offerId = parser.offerId;
               updateItem.price = parser.newPrice;
               --updateItem.offerCount;
               this.var_171.add(parser.offerId,updateItem);
            }
            this.var_171.remove(parser.requestedOfferId);
