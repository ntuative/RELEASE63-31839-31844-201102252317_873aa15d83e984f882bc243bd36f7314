package com.sulake.habbo.moderation
{
   ...
   public class RoomToolCtrl implements IDisposable, TrackedWindow
   {
      ...
      private var var_57:IItemListWindow;
      
      private var _disposed:Boolean;
      
      private var var_1008:IDropMenuWindow;
      
      private var var_519:ITextFieldWindow;
      
      private var var_592:Boolean = true;
      
      private var var_816:ICheckBoxWindow;
      ...
      public function show() : void
      {
         ...
         this.var_33.connection.send(new GetModeratorRoomInfoMessageComposer(this.var_319));
         Logger.log("BEGINNING TO SHOW: " + this.var_319);
      }
      
      public function getType() : int
      {
         return WindowTracker.const_1502;
      }
      ...
      {
         Logger.log("GOT ROOM INFO: " + param1.flatId + ", " + this.var_319);
         if(param1.flatId != this.var_319)
         {
            Logger.log("NOT THE SAME FLAT: " + param1.flatId + ", " + this.var_319);
            return;
         }
         this._data = param1;
         this.populate();
      ...
      {
         this.var_57 = IItemListWindow(this._frame.findChildByName("list_cont"));
         var _loc1_:IWindow = this._frame.findChildByTag("close");
         _loc1_.procedure = this.onClose;
         this.var_519 = ITextFieldWindow(this._frame.findChildByName("message_input"));
         this.var_519.procedure = this.onInputClick;
         this.var_1008 = IDropMenuWindow(this._frame.findChildByName("msgTemplatesSelect"));
         this.prepareMsgSelect(this.var_1008);
         this.var_1008.procedure = this.onSelectTemplate;
         this.var_816 = ICheckBoxWindow(this._frame.findChildByName("kick_check"));
         this.var_1254 = ICheckBoxWindow(this._frame.findChildByName("lock_check"));
         this.var_1253 = ICheckBoxWindow(this._frame.findChildByName("changename_check"));
      ...
      {
         var _loc3_:IWindowContainer = IWindowContainer(this.var_57.getListItemByName(param2));
         ...
         if(!param1.exists)
         {
            this.disposeItemFromList(this.var_57,_loc3_);
            this.disposeItemFromList(this.var_57,this.var_57.getListItemByName("event_spacing"));
            return;
         }
      ...
      private function onChatlog(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_33.windowTracker.show(new ChatlogCtrl(new GetRoomChatlogMessageComposer(0,this._data.flatId),this.var_33,WindowTracker.const_778,this._data.flatId),this._frame,false,false,true);
      }
      ...
      {
         if(this.var_592 || this.var_519.text == "")
         {
            this.var_33.windowManager.alert("Alert","You must input a message to the user",0,this.onAlertClose);
            return;
         }
         var _loc2_:int = this.determineAction(param1,this.var_816.isSelected);
         this.var_33.connection.send(new ModeratorActionMessageComposer(ModeratorActionMessageComposer.const_1429,_loc2_,this.var_519.text,"",""));
         if(this.var_1254.isSelected || this.var_1253.isSelected || this.var_816.isSelected)
         {
            this.var_33.connection.send(new ModerateRoomMessageComposer(this._data.flatId,this.var_1254.isSelected,this.var_1253.isSelected,this.var_816.isSelected));
         }
         this.dispose();
      }
      ...
      private function determineAction(param1:Boolean, param2:Boolean) : int
      {
         if(param2)
         {
            return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1408);
         }
         return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1434);
      }
      ...
      private function onSelectTemplate(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         var _loc3_:String = this.var_33.initMsg.roomMessageTemplates[this.var_1008.selection];
         if(_loc3_ != null)
         {
            this.var_592 = false;
            this.var_519.text = _loc3_;
         }
      }
   }
}
