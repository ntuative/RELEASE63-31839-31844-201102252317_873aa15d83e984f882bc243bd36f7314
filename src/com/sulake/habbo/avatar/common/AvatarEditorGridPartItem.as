package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_757:IFigurePartSet;
      ...
      private const const_1964:int = 13421772;
      
      private const const_1963:int = 16777215;
      
      private const const_330:Array = [2,6,0,4,3,1];
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 != null)
         {
            ...
            {
               this.var_1815 = Math.max(this.var_1815,_loc6_.colorLayerIndex);
            }
         }
         this.var_422 = this.var_65.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_650 != null && !this.var_1814)
            {
               _loc3_ = this.var_650;
            }
            else
            ...
            if(this.var_757)
            {
               switch(this.var_757.clubLevel)
               {
                  ...
                  case HabboClubLevelEnum.const_42:
                     _loc2_.style = HabboIconType.const_841;
               }
            }
            else
            {
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_330[_loc6_] + "_" + FigureData.const_134;
               _loc4_ = this.var_422.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
               while(!_loc7_ && _loc6_ < this.const_330.length)
               {
                  _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_330[_loc6_] + "_" + FigureData.const_134;
                  _loc4_ = this.var_422.getAssetByName(_loc3_) as BitmapDataAsset;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_609 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1816)
               {
                  ...
                  this.var_1816 = _loc9_.content as BitmapData;
               }
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               if(this.var_1814 && _loc2_.colorLayerIndex > 0)
               {
