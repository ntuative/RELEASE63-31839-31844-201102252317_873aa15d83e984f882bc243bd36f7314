package com.sulake.habbo.communication.demo
{
   ...
   public class SSOTicket extends EventDispatcher
   {
      ...
      public function SSOTicket(param1:IAssetLibrary, param2:IHabboWebLogin, param3:String, param4:Boolean = true)
      {
         ...
         this.var_1244 = "http://" + param3 + "/client";
         if(!param4)
         {
            this.var_1243.init();
            this.var_1243.addEventListener(HabboWeb.const_232,this.requestClientURL);
         }
         else
         {
      ...
      {
         var _loc2_:String = this.var_1244;
         if(this._assets.hasAsset(_loc2_))
         {
            Logger.log("[CoreLocalizationManager] reload localization for url: " + this.var_1244);
         }
         var _loc3_:URLRequest = new URLRequest(this.var_1244);
         if(this._assets.hasAsset(_loc2_))
         {
      ...
      private function clientURLComplete(param1:Event = null) : void
      {
         ...
         {
            this.var_1243.requestReAuthenticate();
         }
         else
         {
            try
            {
               this.var_1492 = /\"sso.ticket\" : \"(.*?)\"/.exec(data)[1];
               this.var_1998 = /\"flash.client.url\" : \"(.*?)\"/.exec(data)[1];
               facebookData = /\"facebook.user\" : \"(.*?)\"/.exec(data);
               if(facebookData)
               {
                  this.var_1999 = Boolean(facebookData[1]);
               }
               if(this.var_1492.length > 0)
               {
