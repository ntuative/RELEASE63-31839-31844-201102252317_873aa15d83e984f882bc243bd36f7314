package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private function prepareWindow() : void
      {
         ...
         this.var_1833 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1170 = this.var_1833.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1170)
         {
            this.var_1170.procedure = this.onButtonClick;
         }
         this.var_1171 = this.var_1833.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1171)
         {
            this.var_1171.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_256 >= this.var_78.length - 1)
         {
            this.var_1171.disable();
         }
         else
         {
            this.var_1171.enable();
         }
         if(this.var_256 <= 0)
         {
            this.var_1170.disable();
         }
         else
         {
            this.var_1170.enable();
         }
      }
      ...
      {
         ++this.var_256;
         this.var_256 = Math.min(this.var_256,this.var_78.length - 1);
         this.openPage();
      }
      ...
      {
         --this.var_256;
         this.var_256 = Math.max(this.var_256,0);
         this.openPage();
      }
      ...
      private function localize() : void
      {
         ...
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this.var_78[this.var_256][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this.var_78[this.var_256][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this.var_78[this.var_256][1]).value;
            this.setPreviewFromAsset(_loc4_);
         }
      }
      
      private function method_6(param1:IBitmapWrapperWindow, param2:BitmapData) : void
      {
         if(param2 == null)
         {
            return;
      ...
      private function setPreviewFromAsset(param1:String) : void
      {
         ...
         if(_loc3_)
         {
            this.method_6(_loc3_,_loc2_.content as BitmapData);
         }
      }
