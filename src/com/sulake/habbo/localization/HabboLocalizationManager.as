package com.sulake.habbo.localization
{
   ...
   public class HabboLocalizationManager extends CoreLocalizationManager implements IHabboLocalizationManager
   {
      ...
      private static var var_1245:String = "%";
      ...
      public function HabboLocalizationManager(param1:IContext, param2:uint = 0, param3:IAssetLibrary = null)
      {
         ...
         this.var_2544 = ["I","II","III","IV","V","VI","VII","VIII","IX","X","XI","XII","XIII","XIV","XV","XVI","XVII","XVIII","XIX","XX"];
         super(param1,param2,param3);
         this.var_1781 = (param2 & 0) > 0;
         if(!this.var_1781)
         {
            lock();
         }
      ...
      private function fillParams(param1:String, param2:Dictionary) : String
      {
         ...
         while(_loc3_ < 10)
         {
            _loc4_ = param1.indexOf(var_1245);
            if(_loc4_ < 0)
            {
               break;
            }
            _loc5_ = param1.indexOf(var_1245,_loc4_ + 1);
            if(_loc5_ < 0)
            {
               break;
            }
      ...
      private function onHabboConfigurationInit(param1:IID = null, param2:IUnknown = null) : void
      {
         ...
         if(param2 != null)
         {
            ...
            if(!this.var_1781)
            {
               if("IInventoryView" == this.var_180.getKey("use.default.localizations","IInventoryView"))
               {
                  _loc5_ = this.var_180.getKey("external.texts.txt","external_texts.txt");
                  events.addEventListener(LocalizationEvent.const_158,this.onLocalizationLoaded);
      ...
      {
         var _loc2_:* = null;
         events.removeEventListener(LocalizationEvent.const_158,this.onLocalizationLoaded);
         if(this.var_180.keyExists("external.override.texts.txt"))
         {
            _loc2_ = this.var_180.getKey("external.override.texts.txt");
            events.addEventListener(LocalizationEvent.const_158,this.onLocalizationOverrideLoaded);
      ...
      {
         events.removeEventListener(LocalizationEvent.const_158,this.onLocalizationOverrideLoaded);
         unlock();
      }
      ...
      {
         return this.var_2544[Math.max(0,param1 - 1)];
      }
   }
}
