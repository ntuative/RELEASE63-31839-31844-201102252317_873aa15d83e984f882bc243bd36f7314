package com.sulake.core.window.components
{
   ...
   public class ScrollbarLiftController extends InteractiveController implements IDragBarWindow
   {
      ...
      {
         if(param2.type == WindowNotifyEvent.const_566)
         {
            this._offsetX = x != 0 ? Number(x / Number(0 - width)) : Number(0);
            this.var_1091 = y != 0 ? Number(y / Number(0 - height)) : Number(0);
            if(_parent != this._scrollBar)
            {
               this._scrollBar.update(this,new WindowNotifyEvent(WindowNotifyEvent.const_294,this,null));
            }
         }
